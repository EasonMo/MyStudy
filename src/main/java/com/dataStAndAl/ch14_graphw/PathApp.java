...
class Graph2 {
	...
	public void adjust_sPath() {
		...
		while (column < nVerts) // go across columns
		{
			...
			final int currentToFringe = adjMat[currentVert][column];
			// add distance from start
			final int startToFringe = startToCurrent + currentToFringe;
			// get distance of current sPath entry
			final int sPathDist = sPath[column].distance;
	...
	public void displayPaths() {
		for (int j = 0; j < nVerts; j++) // display contents of sPath[]
		{
			...
			final char parent = vertexList[sPath[j].parentVert].label;
	...
	public void path() // find all shortest paths
	{
		...
		for (int j = 0; j < nVerts; j++) {
			final int tempDist = adjMat[startTree][j];
		...
		while (nTree < nVerts) {
			...
			final int minDist = sPath[indexMin].distance;
